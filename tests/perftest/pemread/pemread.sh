...
parse_args "$@"
# builds check
[ ! -x ${BUILDS_DIR}/${OSSL_VERSION}-tools/perf/pemread ] && echo "Error: The pemread test for OpenSSL ${OSSL_VERSION} not available. Quitting." && exit 1
# metrics setup
METRIC_HOST="PerfTest-OpenSSL-${OSSL_VERSION}"
METRIC_TITLE="perftest.pemread-${THREADS}"
...
for value in ${ALLOWED_THREADS[@]}; do
    ...
    for i in $(seq 1 1 ${REPEAT}); do
        CMD2RUN="LD_LIBRARY_PATH=${BUILDS_DIR}/${OSSL_VERSION} ${BUILDS_DIR}/${OSSL_VERSION}-tools/perf/pemread ${THREADS} | sed 's/^.*: \([0-9]\+\.[0-9]\+\)us\$/\1/'"
